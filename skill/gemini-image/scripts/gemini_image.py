"""
Gemini Image Generator - Skill 独立版本

通过浏览器自动化生成图像，支持 headless 模式。
所有数据（userdata/output）相对于本脚本所在目录的上级存放。
"""
...
class GeminiImageGenerator:
    ...
    def _handle_sign_in(self):
        ...
        if sign_in:
            ...
            try:
                ...
                # 可能跳转到了登录页再回来，等 URL 回到 /app
                try:
                    self.page.wait_for_url("**/app**", timeout=15000)
                    time.sleep(2)
                except Exception:
                    pass
            # 验证是否登录成功（右上角有用户头像/名称）
    ...
    def _count_loading_images(self) -> int:
        return self.page.evaluate("""
        () => document.querySelectorAll('img.image.animate:not(.loaded)').length
        """)
...
def main():
    ...
    p.add_argument("--output", "-o", type=str, default=None, help="输出目录")
    args = p.parse_args()
    ...
    gen = GeminiImageGenerator(headless=not args.no_headless)
    try:
        gen.start()
        if args.prompt:
